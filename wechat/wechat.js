...
var mpprefix='https://mp.weixin.qq.com/cgi-bin/' //通过ticket换取二维码
...
var api={
    ...
    upload: prefix+ 'material/add_material?',
		...
		update:prefix+'update_news?',//access_token=ACCESS_TOKEN
...
Wechat.prototype.deleteMaterial = function(mediaId,type,permanent){
	var that =this
	var form= {
		media_id:mediaId
	}
...
Wechat.prototype.updateMaterial = function(mediaId,type,permanent){
	var that =this
	var form= {
		media_id:mediaId
	}
...
Wechat.prototype.remarkUser = function(id,remark){
	var that =this
	return new Promise(function(resolve,reject){
		that.fetchAccessToken().then(function(data){
		...
		that.fetchAccessToken().then(function(data){
			...
			request({method:'POST',url:url,body:msg,json:true}).then(function(response){
				...
				if(_data){
					...
					throw new Error('send user fails')
				}
			}).catch(function(err){
				reject(err)
			})
		...
		that.fetchAccessToken().then(function(data){
			...
			request({method:'POST',url:url,body:form,json:true}).then(function(response){
				...
				if(_data){
					...
					throw new Error('delete message fails')
				}
			}).catch(function(err){
				reject(err)
			})
...
Wechat.prototype.createQrcode= function(qr){
	...
	return new Promise(function(resolve,reject){
		that.fetchAccessToken().then(function(data){
			...
			request({method:'POST',body:qr,url:url,json:true}).then(function(response){
				var _data=response.body
				if(_data){
					resolve(_data)
				}else{
...
Wechat.prototype.semantic= function(postData){
	var that =this
	return new Promise(function(resolve,reject){
		that.fetchAccessToken().then(function(data){
			var url = semanticUrl+ 'access_token=' +data.access_token
			postData.appid=data.appID
			request({method:'POST',body:postData,url:url,json:true}).then(function(response){
				var _data=response.body
				if(_data){
					resolve(_data)
				}else{
